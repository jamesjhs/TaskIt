...
6. [Using Crystallise](#using-crystallise)
...
**Crystallise** is a self-hosted, collaborative task management application. It runs as a Node.js server with a built-in web front-end and an Android app. Features include:
...
1. **Clone or download** the repository:
   ...
   ```bash
   git clone https://github.com/jamesjhs/Crystallise.git
   cd Crystallise
   ```
...
| `DB_PATH`          | `server/crystallise.db`             | Path to the SQLite database file                         |
...
BASE_URL=https://crystallise.example.com
SMTP_HOST=smtp.example.com
SMTP_PORT=587
SMTP_SECURE=false
SMTP_USER=noreply@example.com
SMTP_PASS=your-smtp-password
SMTP_FROM=Crystallise <noreply@example.com>
```

> **Security note:** Always set a strong, unique `JWT_SECRET` before deploying to production.

> **BASE_URL note:** Set this to your public-facing URL (e.g. `https://crystallise.example.com`) so that invite links, magic links, and QR codes contain the correct address rather than the internal request host.

---

## Encrypting the Database

By default Crystallise stores data in a plain SQLite file. Setting `DB_ENCRYPTION_KEY` in `server/.env` enables full-file encryption via [SQLite3MultipleCiphers](https://utelle.github.io/SQLite3MultipleCiphers/).
...
2. **Run the migration script** from the `server/` directory:
   ...
   Both paths are optional; they default to `server/crystallise.db` → `server/crystallise-encrypted.db`. The script reads `DB_ENCRYPTION_KEY` from `server/.env` — set the key there first before running it.
...
4. **Back up** the original: `cp crystallise.db crystallise.db.bak`

5. **Replace** the original with the encrypted file: `mv crystallise-encrypted.db crystallise.db`
...
Make sure the `sqlite3` binary on your system supports the same cipher (SQLite3MultipleCiphers / sqleet) that Crystallise uses. If not, you can open the database in Node.js and export via `better-sqlite3-multiple-ciphers`'s `backup()` API after applying the key pragma.
...
- **Invite to Crystallise** – copy a shareable link to your Crystallise instance
...
When SMTP is configured and enabled, Crystallise automatically sends reminder emails for each incomplete task that has a due date, according to the **Reminders grid** you configure per task:
...
Crystallise includes an opt-in gamification layer — XP, skill levels, achievements, streaks, and a Freeze mechanic — designed to make staying on top of tasks more rewarding without getting in the way of the core experience.
...
- Set the `BASE_URL` environment variable to your public-facing URL (e.g. `https://crystallise.example.com`) so generated links are correct

---

*Crystallise v1.1.0 – Copyright J Rowson 2026 | jahosi.co.uk*
