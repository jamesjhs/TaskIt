...
- **Persistent login** — web sessions now stay signed in by default for up to 30 days using browser storage; Android uses encrypted app storage
...
- **🔄 PWA arcade cache release fix** — `window.APP_VERSION` is now initialized in the browser shell and refreshed from `/api/version`, so arcade script and Labyrinth iframe URLs use release-specific cache keys instead of falling back to stable `dev` keys. The service worker now precaches versioned frontend assets and serves versioned assets plus standalone pages network-first with cache fallback, reducing the chance that an installed PWA keeps a stale touch-locked game wrapper.
- **🧹 HTML cache revalidation** — all served `.html` files now use `Cache-Control: no-cache`, not just the SPA shell. This keeps standalone iframe pages such as `/labyrinth/labyrinth.html` from remaining stale behind browser, PWA, or CDN caches after a UI interaction fix.
...
- **🔢 Version bump** — package metadata, lockfiles, public cache keys, pages, and documentation updated to 1.22.4.
