 ...
 * encrypt-db.js
 *
 * Migrates an existing plaintext TaskIt! SQLite database to an encrypted one
 ...
 *                  (default: server/taskit.db)
 *   destination  – path for the new encrypted database
 *                  (default: server/taskit-encrypted.db)
 *
 ...
 *   2. Replace server/taskit.db with server/taskit-encrypted.db.
...
const defaultSource = path.join(serverDir, 'taskit.db');
const defaultDest   = path.join(serverDir, 'taskit-encrypted.db');
